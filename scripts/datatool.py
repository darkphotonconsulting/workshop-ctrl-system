...
connection_args.add_argument('--configuration-file',
    type=str,
    required=False,
    action='store',
...
global_args.add_argument('--list-operation-key',
    type=str,
    action='store',
    required=False,
    default=None,
...
global_args.add_argument('--default-schema-template-name',
    type=str,
    action='store',
    default=None,
    required=False,
...
global_args.add_argument('--debug',
    action='store_true',
    required=False,
    help='Print additional debugging messages to the console'
)

global_args.add_argument('--output-file',
    type=str,
    action='store',
    default=None,
    required=False,
...
database_args.add_argument('--list-databases',
    action='store_true',
    required=False,
    help='List databases visible to the configured MongoDB client'
)

database_args.add_argument('--create-database',
    action='store_true',
    required=False,
    help='Create a database with the `--database-name <database_name>` argument as name value'
)

database_args.add_argument('--drop-database',
    action='store_true',
    required=False,
    help='Drop a database with the `--database-name <database_name>` argument as name value'
)
...
collection_args.add_argument('--list-collections',
    action='store_true',
    required=False,
...
collection_args.add_argument('--create-collection',
    action='store_true',
    required=False,
    help=
...
collection_args.add_argument('--drop-collection',
    action='store_true',
    required=False,
    help=
...
collection_args.add_argument('--wipe-items',
    action='store_true',
    required=False,
    help=
...
collection_args.add_argument('--create-if-not-exist',
    action='store_true',
    required=False,
    help='Create a database for a collection if it does not exist, used with `--create-collection`'
)

collection_args.add_argument('--use-default-schema-template',
    action='store_true',
    required=False,
    help=
...
collection_args.add_argument('--collection-name',
    type=str,
    action='store',
    required=False,
    default=None,
...
collection_args.add_argument('--seed-data',
    type=str,
    action='store',
    required=False,
    help='Path to a file containing the seed data JSON file in an appropriate format if applicable'
)


misc_args.add_argument('--print-default-template',
    action='store_true',
    required=False,
    help='Prints the default schema requested by `--default-schema-template-name <schemaName>`'
)

misc_args.add_argument('--write-default-template',
    action='store_true',
    required=False,
    help='Write the default schema requested by `--default-schema-template-name <schemaName>` to a file defined by --output-file <path/to/outputfile.json>'
)
...
def get_engine(
    args: Namespace
) -> Union[SchemaMigrationEngine,bool]:
...
def write_seed_data(
    ...
    if args.seed_data in ['system', 'gpios', 'relays']:
        ...
        logger.warning(f"the argument --seed-data is {args.seed_data} but must be one of ...")
        return False
...
def list_collections(
    ...
    if engine.validate_connection():
        if args.database_name is not None:
            ...
            logger.warning(f"please set the database name using --database-name <dbName>")
            return False
...
def create_database(
    ...
    if args.create_database and args.database_name is not None:
        ...
        logger.warning("In order to use create database, please provide the --database-name <dbName> argument")
        return False

def drop_database(args: Namespace
    ) -> bool:
    engine = get_engine(args)
    if args.database_name is not None:
        return engine.drop_database(
            database_name=args.database_name
        )
    else:
        logger.warning(f"please set the --database-name <dbname> in order to use --drop-database")
...
def create_collection(
    ...
    if args.create_collection and args.database_name is not None and args.collection_name is not None:
        ...
        if args.use_default_schema_template:
            template = engine.get_default_schema_template(schema_template_name=args.default_schema_template_name)
        ...
        logger.warning(f"please set --database-name <dbName> --collection-name <collectionName> in order to use --create-collection")

def drop_collection(args: Namespace
) -> bool:
    engine = get_engine(args)
