    ...
    if isinstance(data, dict):
        for k, v in data.items():
            if k == key:
                ...
                nested_results = deepfind(v, key, val)
                for res in nested_results:
                    results.append(res)
    return results


def deepsearch(data, search, keys=True, object=False):
    results = []
...
def api_pi_gpio_data():
    ...
    if all(k in args for k in ['find_by', 'value']):
        uresults = []
        find_by = args.get('find_by')
        value = args.get('value')
        try:
            for boardpin, gpio in results.items():
                res = deepfind(gpio, find_by, value)
                if len(res) > 0:
                    uresults.append(gpio)
                # if find_by in gpio:
                #     if gpio[find_by] == value:
                #         uresults.append(gpio)
            return jsonify(uresults)
        except:
            print('ok')
    if "physical" in args:
        try:
            results = results[args.get('physical')]
